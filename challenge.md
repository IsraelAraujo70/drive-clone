...
| ------------------------------ | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| Authentication                 | Done                  | `POST /auth/signup`, `POST /auth/login`, `GET /auth/me`; Cypress signs up through the UI and API helpers cover setup.                                                                            | None for MVP.                                                                                                                                      |
| File upload                    | Done                  | Resumable multipart upload is the primary UI path; direct upload remains for compatibility; Cypress uploads through the browser against MinIO.                                                   | Production smoke after the latest commits.                                                                                                         |
...
| File download                  | Done                  | Authorized signed downloads for owner and grantee; Cypress byte-compares downloaded content.                                                                                                     | None for MVP.                                                                                                                                      |
| Unauthorized access prevention | Done                  | Cypress covers private file denial, shared access, trashed-file denial, and ACL-scoped search.                                                                                                   | None for MVP.                                                                                                                                      |
...
| Folder management              | Done                  | Create, browse, rename, move, recursive trash, restore, and invalid cycle rejection are covered by Rust use-case tests.                                                                          | None for MVP.                                                                                                                                      |
| Trash restore                  | Done                  | File and folder restore from trash in API and UI.                                                                                                                                                | None for MVP.                                                                                                                                      |
| Force delete from trash        | Done                  | UI `Force delete`; `DELETE /files/{file_id}/purge`; `DELETE /folders/{folder_id}/purge`; Rust tests plus Cypress trash flow.                                                                     | Production smoke after deploy.                                                                                                                     |
...
| Public share links             | Done                  | Create/list/revoke/resolve public links with expiry and uniform 404 on invalid/revoked/expired/trashed targets; Rust use-case tests and Cypress selectors for the UI path.                       | None for MVP.                                                                                                                                      |
...
| Search                         | Done                  | Filename search, ACL scoped, deleted excluded by default, owner's trash included only with `include_deleted=true`; Cypress verifies owner and grantee search scopes.                             | None for MVP.                                                                                                                                      |
| Sync change feed               | Done for contract/API | `GET /sync/changes` emits upserts and tombstones with stable cursor and pagination; Rust use-case tests cover ordering.                                                                          | No desktop or CLI sync client yet. Conflict handling is specified at a high level, not implemented in a real client.                               |
| Resumable uploads              | Done                  | Create session, upload parts, status endpoint, finalized multipart object, abandoned upload expiry, local resume UI using the same selected file; Cypress covers browser recovery through MinIO. | Production resume smoke after latest deploy.                                                                                                       |
...
| Automated tests                | Done                  | Rust unit/use-case/domain tests, web Vitest tests, and Cypress full-stack e2e in TypeScript.                                                                                                     | Keep adding tests with feature changes.                                                                                                            |
| End-to-end tests               | Done                  | Cypress runs browser -> Next.js -> Rust API -> Postgres -> MinIO from Docker Compose.                                                                                                            | Wire the same suite into CI/CD.                                                                                                                    |
