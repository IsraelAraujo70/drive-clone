  ...
  },
  {
    ...
    requests: [
      ...
      },
      {
        ...
        docs: "Requests a one-hour password reset link. Unknown email still returns 204 so account existence is not leaked.",
        tests: ["expect([204, 422, 502]).to.include(res.status);"],
      },
      {
        name: "Reset Password",
        method: "post",
        ...
        docs: "Consumes a password reset token, changes the password, and revokes existing sessions.",
        tests: ["expect([204, 422]).to.include(res.status);"],
      },
      {
        name: "Logout",
        method: "post",
    ...
    requests: [
      {
        ...
        docs: "Owner-only. Creates a public, revocable read-only link to a single complete file. Set expires_in_seconds to a positive integer for an expiring link, or null for one that never expires; <= 0 returns validation_error. The plaintext token is returned only here.",
        postResponse: [
          "if (res.body && res.body.id) {",
      ...
      },
      {
        ...
        url: "{{baseUrl}}/files/{{fileId}}/share-links",
        auth: "bearer",
        docs: "Owner-only list of the file's share links. Tokens are never returned; each entry carries id, created_at, expires_at, and revoked_at.",
        tests: ["expect([200, 401, 404]).to.include(res.status);"],
      },
      {
        name: "Revoke Share Link",
        method: "delete",
        url: "{{baseUrl}}/files/{{fileId}}/share-links/{{shareLinkId}}",
        auth: "bearer",
        docs: "Owner-only. Sets revoked_at (row kept for audit). Unknown, already-revoked, or cross-user link returns file_not_found.",
        tests: ["expect([204, 401, 404]).to.include(res.status);"],
      },
      {
        name: "Resolve Public Link",
        method: "get",
        url: "{{baseUrl}}/shared/links/{{shareLinkToken}}",
        auth: "none",
        docs: "Public, unauthenticated. Resolves a valid token to filename, size_bytes, content_type, and a presigned download_url. Any failure (bad token, revoked, expired, trashed file) returns a uniform 404 file_not_found.",
        tests: ["expect([200, 404]).to.include(res.status);"],
      },
    ],
  },
];
